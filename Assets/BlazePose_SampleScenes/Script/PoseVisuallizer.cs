...
{
    [SerializeField] RenderTexture webCamInput;
    ...
    void Update(){
        ...
        cmd.DrawProcedural(Matrix4x4.identity, material, 0, MeshTopology.Triangles, 6, BODY_LINE_NUM);
        Graphics.ExecuteCommandBuffer(cmd);
        cmd.Release();

        material.SetPass(1);
        cmd = new CommandBuffer();
        cmd.DrawProcedural(Matrix4x4.identity, material, 0, MeshTopology.Triangles, 6, detecter.vertexCount);
        Graphics.ExecuteCommandBuffer(cmd);
        cmd.Release();
    } 
