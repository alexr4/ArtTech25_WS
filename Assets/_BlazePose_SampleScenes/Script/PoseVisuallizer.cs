...
public class PoseVisuallizer : MonoBehaviour
{
    ...
    public SampleHandPositions sample;
    ...
    void LateUpdate(){
        inputImageUI.texture = rt;

        // Predict pose by neural network model.
        detecter.ProcessImage(rt);
        ...
        //Debug.Log("---");
        //Debug.LogFormat("{0}: {1}", 15, detecter.GetPoseLandmark(15));

        if (prevRightHand == null) prevRightHand = detecter.GetPoseLandmark(15);

        if (prevLeftHand == null) prevLeftHand = detecter.GetPoseLandmark(16);

        //sample.AddHandPosition(detecter.GetPoseWorldLandmark(15));
        sample.Trace(prevRightHand, detecter.GetPoseLandmark(15));
        sample.Trace(prevLeftHand, detecter.GetPoseLandmark(16));

        prevRightHand = detecter.GetPoseLandmark(15);
        prevLeftHand = detecter.GetPoseLandmark(16);
    } 
